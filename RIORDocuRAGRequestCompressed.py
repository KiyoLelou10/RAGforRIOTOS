#!/usr/bin/env python3
"""
RIOT OS Documentation Compressed Embedding Search System

This script searches through compressed RIOT OS documentation chunks using
the compressed embeddings and trained autoencoder model.

Dependencies:
    pip install torch numpy sentence-transformers scikit-learn

Usage:
    python riot_compressed_search.py "How do I configure GPIO pins in RIOT?"
    python riot_compressed_search.py "RIOT networking stack usage" --module sys
    python riot_compressed_search.py "Timer configuration" --output prompt.txt
...
class RIOTCompressedSearchSystem:
    """Search system for compressed RIOT OS documentation embeddings."""
    ...
    def __init__(self, chunks_file: str,
        ...
        logger.info(f"Search system initialized with {len(self.chunks)} chunks")
    ...
    def _load_autoencoder(self) -> EmbeddingAutoencoder:
        ...
        logger.info(f"Loaded autoencoder: {checkpoint['input_dim']} -> {checkpoint['compressed_dim']}")
        return model
    ...
    def _prepare_embeddings_matrix(self) -> np.ndarray:
        ...
        for chunk in self.chunks:
            embedding = np.array(chunk['compressed_embedding'], dtype=np.float32)
            embeddings.append(embedding)

        embeddings_matrix = np.vstack(embeddings)
        logger.info(f"Prepared embeddings matrix: {embeddings_matrix.shape}")
        return embeddings_matrix

    def _enhance_query(self, query: str) -> str:
        """Enhance query with RIOT-specific context (same as original system)."""
        return f"RIOT OS documentation query: {query}"

    def _encode_query(self, query: str) -> np.ndarray:
        """Encode query using the embedding model and compress with autoencoder."""
        # Enhance query
        ...
        with torch.no_grad():
            ...
            compressed_query = self.autoencoder.encode(query_tensor)
            compressed_embedding = compressed_query.cpu().numpy().flatten()
    ...
    def search_chunks(self, query: str,
        ...
        """Search for relevant chunks using compressed embeddings."""
        ...
        for i, (sim_score, div_score, pref_score) in enumerate(
            ...
            combined_score = (
                    0.6 * sim_score +  # Primary: semantic similarity
                    0.2 * div_score +  # Secondary: diversity
...
The following {len(chunks)} chunks were retrieved from the official RIOT OS documentation using compressed embeddings based on semantic similarity and relevance to your question:
...
"""
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "original_embedding_dimension": self.autoencoder.input_dim,
            "compression_ratio": f"{self.autoencoder.input_dim}:{self.autoencoder.compressed_dim}",
...
def main():
    """Main function for the compressed embedding search system."""
    parser = argparse.ArgumentParser(
        description='RIOT OS Documentation Compressed Embedding Search System',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python riot_compressed_search.py "How do I configure GPIO pins?"
  python riot_compressed_search.py "RIOT networking stack" --module sys --chunks 10
  python riot_compressed_search.py "Timer configuration" --output prompt.txt
  python riot_compressed_search.py "Thread synchronization" --category "core"
        """
    )

    parser.add_argument('query', help='Your question about RIOT OS')
    parser.add_argument('--chunks-file', default="compressed_chunks.json",
                        help='Path to compressed chunks JSON file')
    ...
    try:
        ...
        search_system = RIOTCompressedSearchSystem(
