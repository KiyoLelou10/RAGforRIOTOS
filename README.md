...
# Documentation RAG
It is important to follow this work Flow, https://github.com/RIOT-OS/RIOT is where the RIOT lies please go ahead and clone it. Then use doxygen to generate the documentation of RIOT locally. 
First we need to create chunks from RIOT's documentation go ahead and run python3 RIOTDocuChunker2.py path/to/local/riot/repo/documentation, afterwards if you left the defaults a new json called riot_chunks.json should be generated which contains RIOT's documentation divided into overlapping chunks with some metadata. 
Then run python3 RIOTRRAGDocuDB3.py riot_chunks.json (or whatever name you gave to the json), this should embed the chunks into vector that capture the semantic meaning, these chunks alongside their embedding is saved in a chromadb, by default it is ./riot_vector_db.
If this succeeds we can now use our rag with python3 RIORDocuRAGRequest2.py "your query", this will retrieve the most relevant chunks from the documentation that "match" (where the vectors align the most) the user query (we essentially build the cosine similarity between embedding of query and embedding of each chunk). This will return a beforehand best practice prompt that includes your original query and the chunks you can copy it and give it to an LLM of your choice.

# Autencoder
We can try to speed up our RAG by compressing the vectors used for representation of data, we also hope to maybe improve the quality of the RAG by compressing the vectors, right now we are have 768 dimensions for each vector (mpnet embedding space), we now train an autencoder with the all the chunks' embeddings to maybe compress the vectors into a smaller dimension portraying the most important information in a more concise form, for the queries I tested personally, I did not see any noteworthy improvement using the Autoencoder, but you can try out yourself and check the results.
First run python3 AutoencoderRIOT2.py (if you have the chromadb saved at the default location), and you can check the progress yourself, alternatively you can also run python3 AutoencoderRIOTTriplet.py --epochs 100 --lambda-triplet 5.0 --margin 1.5 to create some grouping of the encoded vectors using anchors, again I did not see any improvements here either myself. To make requests to these to run python3 RIORDocuRAGRequestCompressed.py "your query" or python3 RIORDocuRAGRequestCompressedTriplet.py "your query" again assuming you used the default location for saving weights and chunks. This will like the other request script, give back a full prompt you can give to the LLM of your choice

# Code RAG
This is an additional part which also has a RAG for the example directory of RIOT, first in chunker.py change the directory path line 11 to the path on your computer, then run python3 chunker.py, afterwards run python3 embedder.py, now you have chunks with embeddings of the example directory, running python3 request.py "your query" will again give you the chunks that "match" your query the most, this is just an additional help that can help in some cases, be careful when using though because if there is no example yet for your query, this may very well return completely irrelevant chunks giving them to chatgpt instead of just your query or maybe just the query the other RAG gave you can become counterproductive 
